...
int CAN_send(unsigned int id, unsigned int length, unsigned int data_h, unsigned int data_l) {
    ...
    while (!((CAN_TransmitStatus(TransmitMailbox) == CANTXOK) || (CAN_TransmitStatus(TransmitMailbox) == CANTXFAILED)));

    return CAN_TransmitStatus(TransmitMailbox);

/*
    i = 0;
    while((CAN_TransmitStatus(TransmitMailbox) != CANTXOK) && (i != 0xFFFFFF))
        i++;

    i = 0;
    while((CAN_MessagePending(CAN_FIFO0) < 1) && (i != 0xFFFFFF))
        i++;
